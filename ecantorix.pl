...
EOF
	...
	wav => {
		...
		sample => sub {
			...
			close $fh
				or die "$cmd: $! $?";
			die "IN=$outname RATE=$SOX_RATE AFTEREFFECTS_CHANNELS=$SOX_AFTEREFFECTS_CHANNELS $cmd: No data"
				unless length $data;
...
my $ass_line = "";
my $ass_starttime = undef;  # Start time of line.
my $ass_lasttime = undef;  # End time of last lyric.
my $ass_endtime = undef;  # End time of line.
my $ass_have_header = 0;
my $ass_spacebuffer = "";
sub ass_header()
{
	return if $ass_have_header;
	$ass_have_header = 1;
	print <<EOF;
[Script Info]
...
sub ass_endofline($)
{
	my ($nextstart) = @_;
	return
		if not defined $ass_lasttime;

	$ass_line .= "{\\k0}}"
		if $ass_spacebuffer ne '';
	$ass_line .= $ass_spacebuffer;
	$ass_spacebuffer = '';

	ass_header();

	# Advance to the next line of lyrics.
	if (defined $nextstart)
	{
		my $d = $nextstart - $ass_lasttime;
		$d = $ASS_PRETIME + $ASS_POSTTIME
			if $d > $ASS_PRETIME + $ASS_POSTTIME;
		$d = 0
			if $d < 0;
		$d *= $ASS_POSTTIME / ($ASS_PRETIME + $ASS_POSTTIME);
		$ass_endtime = $ass_lasttime + $d;
	}
	else
	{
		$ass_endtime = $ass_lasttime + $ASS_POSTTIME;
	}
	my $startstr = hmmsshh $ass_starttime;
	my $endstr = hmmsshh $ass_endtime;
	statusout "";
...
sub ass_lyric($$$)
{
	...
	my $pre_newline = $text =~ s/^\s*\n\s*//;
	my $post_newline = $text =~ s/\s*\n\s*$//;

	$text =~ s/^(\s*)(.*?)(\s*)$/$2/;
	my $pre_space = $1;
	my $post_space = $3;

	# Buffer the whitespace before this lyric.
	$ass_spacebuffer .= $pre_space;
	ass_endofline($start)
		if $pre_newline;

	# Open a new line, if necessary
	if (!defined $ass_lasttime)
	{
	...
	$ass_line .= "{\\k$ass_delta}"
		if $ass_delta > 0 || $ass_spacebuffer ne '';
	$ass_line .= $ass_spacebuffer;
	$ass_spacebuffer = '';
	...
	$ass_line .= "{\\kf$ass_length}"
		if $ass_length > 0 || $text ne '';
	$ass_line .= $text;
	$ass_lasttime += 0.01 * ($ass_delta + $ass_length);

	# Buffer the whitespace after this lyric.
	$ass_spacebuffer .= $post_space;
	ass_endofline($end)
		if $post_newline;
}
...
sub split_lyric($) {
	...
	$text =~ /(.*)(\[\[.*\]\])(.*)$/s
		or return ($text, $text, $is_chord);
	return "$1$3", $2, $is_chord;
}
...
for my $trackno(0..@$tracks-1)
{
	...
	for (@events) {
		...
		my $start = $_->[1];
		my $text = $_->[2];
		#use Data::Dumper; statuswarn Dumper "$text";
		$text =~ s/^[\/\\]\s*/\n/;
		next
			if $text =~ /^\s*%/;
		next
	...
	for(@lyrics)
	{
		...
		my ($lyric, $speech, $is_chord) = split_lyric $text;
		...
		for my $channel(sort keys %$channels)
		{
			...
			ass_lyric $realstarttime, $realendtime, $lyric;
			next
				if not defined $out;
			if ($is_chord) {
				...
				for (@$notes) {
					...
					die "Chord with non-overlapping notes: $notestarttick/$noteticks/$pitch/$velocity vs $chordstarttick/$chordticks/$chordpitch/$chordvelocity - will cycle"
						if $notestarttick >= $chordstarttick + $chordticks || $notestarttick + $noteticks <= $chordstarttick;
					...
					if ($notestarttick > $chordstarttick) {
						$chordticks -= $notestarttick - $chordstarttick;
						$chordstarttick = $notestarttick;
					}
					if ($notestarttick + $noteticks < $chordstarttick + $chordticks) {
						$chordticks = $notestarttick + $noteticks - $chordstarttick;
					}
					push @pitches, $pitch;
				}
				my $i = 0;
				...
				for(@$notes)
				{
					...
					my $notestarttime = tick2sec($notestarttick);
					my $noteendtime = tick2sec($noteendtick);
					die "Overlapping notes: $lastnoteendtime/$notestarttime/$noteendtime/$channel/$pitch/$lyric/$speech"
